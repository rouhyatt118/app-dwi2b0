...
antsMultivariateTemplateConstruction2.sh   -d ${DIMENSION}   -c 0  -n 0  -q  100x100x0x0  -j ${CPU_CORES}  -g ${GRADIENT_STEP}  -i ${ITERATIONS}  -k 1  -m CC[4]  -t Rigid[${GRADIENT_STEP}]  -o "${OUTPUT_PREFIX}" "${INPUT_IMAGES[@]}"
